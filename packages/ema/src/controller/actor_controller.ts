...
export class ActorController {
  ...
  async get(actorId: number): Promise<ActorDetails | null> {
    ...
    const latestPreview = await this.getLatestPreview(actor.id);
    return {
  ...
  async listForUser(userId: number): Promise<ActorDetails[]> {
    ...
    const actors = await Promise.all(
      relations.map((relation) => this.get(relation.actorId)),
    );
  ...
  private async getLatestPreview(actorId: number) {
    const latest =
