...
export function createBootstrapConfig(
  ...
  const mode = input.mode ?? parseMode(env("EMA_MODE")) ?? "prod";
  const explicitMongoUri = input.mongoUri ?? env("EMA_MONGO_URI") ?? "";
  const mongoKind =
    input.mongoKind ??
    parseMongoKind(env("EMA_MONGO_KIND")) ??
  ...
  const dataRoot = resolveWorkspacePath(
    input.dataRoot ?? env("EMA_DATA_ROOT") ?? DEFAULT_DATA_ROOT,
  );
