...
function main() {
  ...
  const nextScript = mode === "dev" ? "dev" : "prod";
  ...
  const env = {
    ...process.env,
    EMA_MODE: mode,
    EMA_MONGO_KIND: mode === "dev" && !mongoUri ? "memory" : "remote",
    EMA_MONGO_URI: mongoUri,
    EMA_MONGO_DB: MONGO_DB_NAME,
    EMA_DATA_ROOT: dataRoot,
  };
